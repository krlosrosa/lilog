...
import { 
  Boxes, CalendarIcon, Warehouse, ScanLine, Clock, 
...
const CenterRankingChart = ({ data }: { data: any[] }) => (
  <Card>
    <CardHeader>
      <CardTitle>Ranking de Produtividade por Centro</CardTitle>
...
const ClusterComparisonChart = ({ data }: { data: any[] }) => (
  <Card>
    <CardHeader>
      <CardTitle>Comparativo por Cluster</CardTitle>
      <CardDescription>Produtividade por cluster.</CardDescription>
    </CardHeader>
    <CardContent>
      <ChartContainer config={{}} className="h-[320px] w-full">
        <ResponsiveContainer>
          <BarChart data={data}>
            ...
            <Bar dataKey="produtividade" fill="hsl(var(--chart-3))" radius={[4, 4, 0, 0]} name="Produtividade" />
          </BarChart>
        </ResponsiveContainer>
      </ChartContainer>
    </CardContent>
...
const DonutChartCard = ({ data, title, description }: { 
  data: any[];
  title: string;
  description: string;
}) => (
        ...
        {data.map((item, index) => (
          <div key={index} className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-medium">{item.centro}</p>
                <p className="text-xs text-muted-foreground">{item.cluster} • {item.totalCaixas.toLocaleString()} caixas</p>
              </div>
            </div>
...
export default function DashboardPage() {
  ...
  const { data, isLoading, error } = useDashCentros({ 
    dataInicio, 
    dataFim 
  });
  ...
  // Agregar dados por cluster para comparação
  const clusterData = data.rankingProdutividadePorCluster.reduce((acc, item) => {
    const existing = acc.find(x => x.cluster === item.cluster);
    if (existing) {
      existing.totalCaixas += item.totalCaixas;
      existing.horasTrabalhadas += item.horasTrabalhadas;
    } else {
      acc.push({
    ...
    return acc;
  }, [] as { cluster: string; totalCaixas: number; horasTrabalhadas: number; produtividade: number }[]);

  // Calcular produtividade média por cluster
  clusterData.forEach(item => {
    item.produtividade = item.horasTrabalhadas > 0 
      ? Number((item.totalCaixas / item.horasTrabalhadas).toFixed(1))
      : 0;
  });
  ...
  return (
    <div className="flex-1 space-y-6 p-4 md:p-8 pt-6">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <ClusterComparisonChart data={clusterData} />
        <CenterRankingChart data={data.rankingProdutividadePorCentro} />
      </div>

      <div className="grid gap-6 md:grid-cols-2">
        <DonutChartCard 
          data={data.rankingProdutividadePorCentro}
          title="Distribuição de Caixas por Centro" 
          description="Percentual de caixas processadas por centro." 
        />
        <DonutChartCard 
          data={clusterData}
          title="Distribuição de Caixas por Cluster" 
          description="Percentual de caixas processadas por cluster." 
        />
      </div>
    </div>
  );
