...
export const TableRowPicking = ({ item, index, config }: TableRowProps) => {
  return (
    <tr
      className={`${
    ...
    >
      ...
      {config?.dataMaximaPercentual != null && config.dataMaximaPercentual > 0 && <RowCell extraClass="w-[10%]">
...
function RowCell({
  ...
  return (
    <td
      className={`border-b border-r border-gray-300 p-0.5 text-center ${extraClass}`}
    >
      {children}
    </td>
  )
